...
window.formatGoogleCalendar = function () {
    ...
    var transformationList = function transformationList(result, tagName, format) {
        ...
        var dateFormatted = getFormattedDate(dateStart, dateEnd, dayNames, moreDaysEvent, isAllDayEvent),
			...
			// https://bencollier.net/2011/05/quickly-creating-an-html-link-in-javascript/
			text = 'more info',
			link = text.link(description),
			i;
			
			// have to put white space in link in description 
			description = description.replace(/\s+/g,'');
			link = text.link(description);
			
			//output = output + '<a href="' + 'https://www.facebook.com/events/144277686378937/' + '" target="_blank" >';
			//output = output + '<a href="index.html" onclick="location.href=this.href+'?key='+ ' + description + ' ;return false;" target="_blank" >';
        ...
        for (i = 0; i < format.length; i++) {
            ...
            if (format[i] === '*summary*') {
				...
				//output = output.concat('<span class="description"><a id="link" href=""> Link </a></span>');
				//output = output.concat(description);
				//output = output + string(description);
				//output = output.concat(link);
				output = output.concat('<span class="description">' + link + '</span>');
            } else if (format[i] === '*location*') {
                output = output.concat('<span class="location">' + location + '</span>');
			} else if (format[i] === '*htmlLink*') {
                output = output.concat('<span class="htmlLink">' + htmlLink + '</span>');
            } else {
                if (format[i + 1] === '*location*' && location !== '' || format[i + 1] === '*summary*' && summary !== '' || format[i + 1] === '*date*' && dateFormatted !== '' || format[i + 1] === '*description*' && description !== '' || format[i + 1] === '*htmlLink*' && htmlLink !== '') {
                    output = output.concat(format[i]);
                }
            }
			
		//document.getElementById('link').innerHTML = '<a href="' + description + '">Link</a>';
		//var a = document.getElementById('link'); //or grab it by tagname etc
		//a.setAttribute("href", description);
    ...
    return {
        init: function init(settingsOverride) {
            var settings = {
                calendarUrl: 'https://www.googleapis.com/calendar/v3/calendars/imtvdia40iq5d7q9u8bcn92huc@group.calendar.google.com/events?key=AIzaSyCbmAGnO4eC8iluuk4SrIRcaQI6qjctDPY',
                past: true,
                upcoming: true,
                sameDayTimes: true,
                dayNames: true,
